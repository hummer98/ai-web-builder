...
- **インフラ方針:** Fly.io（編集環境） + Cloudflare / Firebase（本番公開）の2本柱（ゲストサイトの設定ファイルで自動振り分け）
...
│  │                         │  │                                    │    │
│  │  [?] [履歴] [↩戻す] [公開]│  │  [Inspect] [Desktop|Tablet|Mobile] │    │
│  │                         │  │                                    │    │
│  │  💬 チャット              │  │  ┌──────────────────────────────┐ │    │
│  │  📎 画像添付             │  │  │  ゲストサイト (Vite HMR)      │ │    │
│  │                         │  │  │                              │ │    │
│  │  コマンド検出:            │  │  │  Inspect モード:             │ │    │
│  │  「元に戻して」→ undo     │  │  │  ・ホバー → 日本語ラベル     │ │    │
│  │  「公開して」→ deploy     │  │  │  ・クリック → コンテキストメニュー│ │    │
│  │  「使い方」→ ヘルプ       │  │  │    テキスト編集 / 画像差替    │ │    │
│  │  「〇〇のサイトを作って」  │  │  │    削除 / チャットで指示      │ │    │
│  └────────┬────────────────┘  │  └──────────────────────────────┘ │    │
│           │                   │         │ postMessage               │    │
│           │                   └─────────┼───────────────────────────┘    │
└───────────┼─────────────────────────────┼───────────────────────────────┘
            ...
            ▼                             ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  Fly.io コンテナ (Auto Start/Stop, NRT)                                  │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────┐            │
│  │  Agent Server :8080 (Hono)                               │            │
│  │                                                           │            │
│  │  HTTP:  /health, /api/upload, /preview/* (→Vite proxy)    │            │
│  │  WS:    chat, undo, deploy, history, revert,              │            │
│  │         create-site, import-repo                          │            │
│  │                                                           │            │
│  │  ┌──────────────┐  ┌─────────────────────────────┐       │            │
│  │  │ detectCommand│  │ handleCommand               │       │            │
│  │  │ (自然言語→   │→ │ undo / deploy / help /      │       │            │
│  │  │  コマンド変換)│  │ create / import             │       │            │
│  │  └──────────────┘  └─────────────────────────────┘       │            │
│  │         │ (コマンド以外)                                    │            │
│  │         ▼                                                  │            │
│  │  ┌───────────────────────────────────┐                    │            │
│  │  │ OpenCode SDK (promptAsync + SSE)  │                    │            │
│  │  │ → ストリーミングで WS に中継       │                    │            │
│  │  │ → 完了時に autoCommit + autoPush  │                    │            │
│  │  └──────────────┬────────────────────┘                    │            │
│  └─────────────────┼─────────────────────────────────────────┘            │
│                    ▼                                                      │
│  ┌──────────────────────────────────────────────────────────┐            │
│  │  opencode serve :4096 (AI 編集エンジン)                    │            │
│  │                                                           │            │
│  │  MCP サーバー:                                             │            │
│  │  ┌─────────────────┐  ┌───────────────────────────┐      │            │
│  │  │ Playwright MCP  │  │ log-reader MCP             │      │            │
│  │  │ (Chromium)      │  │ read_log / search_log /    │      │            │
│  │  │ → screenshot    │  │ list_logs                   │      │            │
│  │  │ → console_msgs  │  │ → logs/ 横断検索            │      │            │
│  │  │ → 視覚検証      │  │                             │      │            │
│  │  └────────┬────────┘  └──────────────┬────────────┘      │            │
│  │           ▼                          ▼                    │            │
│  │  自己修復ループ (OPENCODE.md):                              │            │
│  │  編集 → screenshot → console → logs → エラーなら自動修正   │            │
│  └──────────────────────────────────────────────────────────┘            │
│                    │ ファイル編集                                          │
│                    ▼                                                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────────────────────────────┐       │
│  │  Vite    │  │  Hono    │  │  Fly Volume (/data)              │       │
│  │  :5173   │  │  :3000   │  │  workspace/                      │       │
│  │  (HMR)   │  │  (API)   │  │  ├── src/        → Vite HMR     │       │
│  └──────────┘  └──────────┘  │  ├── functions/   → tsx watch    │       │
│                               │  ├── public/uploads/ (画像)     │       │
│                               │  └── .git/       → GitHub push  │       │
│                               └──────────────────────────────────┘       │
...
│                    │  │                    │  │                    │
│  ai-web-builder    │  │  Access (認証)     │  │  LLM API           │
│  [bot] 自動コミット │  │  Pages (静的)      │  │  Gemini 2.5 Pro    │
│  push / revert     │  │  Workers (API)     │  │  等                │
└───────────────────┘  └───────────────────┘  └───────────────────┘
```
...
```
Fly.io (編集環境)
├── ai-web-builder        (本番: editor.le-serpent.club)
│   └── Machine + Volume   SITE_DOMAIN=le-serpent.club
│
└── ai-web-builder-demo   (デモ: ai-web-builder-demo.fly.dev)
    └── Machine + Volume   SITE_DOMAIN 未設定 → scaffold

本番公開先 (ゲストサイトごとに自動振り分け)
├── Cloudflare Pages + Workers   (workspace/wrangler.toml が存在する場合)
└── Firebase Hosting + Functions (workspace/firebase.json が存在する場合)
    └── le-serpent.club  (Hosting + Functions asia-northeast1)
```
...
| 本番ホスティング | Cloudflare（Pages + Workers + D1） または Firebase（Hosting + Cloud Functions、asia-northeast1） — workspace 直下の `wrangler.toml` / `firebase.json` で自動振り分け |
...
| 役割 | エディターモード (Fly.io) | 本番 (Cloudflare / Firebase) |
|------|--------------------------|------------------------------|
| フロントエンド | Vite Dev Server :5173 | Pages（静的ファイル） / Hosting |
| バックエンド API | Hono Dev Server :3000 | Workers / Cloud Functions（asia-northeast1） |
| データベース | D1（HTTP経由） | D1（ネイティブ） / — (Firebase 経路では未使用) |
...
ユーザーが扱う API 鍵は **BYOK (Bring Your Own Keys)** とし editor の設定画面から登録する。サーバー側の運用鍵（GitHub App）のみ Fly Secrets / `.envrc` で扱う。

#### 5.3.1 BYOK 対象（editor 設定画面 → `/data/secrets.json`）

| プロバイダ | 用途 | フィールド | 取得先 |
|----------|------|----------|--------|
| OpenRouter | LLM API（OpenCode 経由）— 必須 | `apiKey` | [openrouter.ai/keys](https://openrouter.ai/keys) |
| Gemini | 画像生成（任意） | `apiKey` | [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey) |
| Cloudflare | wrangler デプロイ（Pages/Workers/D1） | `apiToken` + `accountId` | [dash.cloudflare.com/profile/api-tokens](https://dash.cloudflare.com/profile/api-tokens) |
| Firebase | firebase-tools デプロイ（Hosting/Functions） | `token` | `firebase login:ci` で発行 |

**保管:** `/data/secrets.json` (0600 / atomic write / `.gitignore` 済)。`SECRETS_FILE` env で上書き可。ローカル開発では `<repo>/data/secrets.json` にフォールバック。

**HTTP API:** `GET/PUT/DELETE /api/secrets` (`container/agent-server/src/api-secrets.ts`)。レスポンスに鍵本体を含まず、status は `set` boolean と `last4` のみ。

**注入:**

- OpenCode は `opencode-supervisor` が secrets.json から openrouter / gemini を読んで `opencode.json` に postprocess (`--from-secrets`) で書き込む
- spawn 時は `buildSanitizedEnv()` で `OPENROUTER_API_KEY` / `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` を `process.env` から削除（env 経由の漏洩を二重防御）
- wrangler / firebase は `deploy.ts` 内で `loadSecrets()` から env を組み立てて spawn。未登録時は `cloudflare_secrets_not_configured` / `firebase_secrets_not_configured` で build 前にガード
- openrouter / gemini を更新すると opencode が自動再起動

#### 5.3.2 サーバー側鍵（Fly Secrets / `.envrc`）

| 変数名 | 用途 | 取得先 |
|--------|------|--------|
| `GITHUB_APP_ID` | GitHub App 識別 | App 作成時に発行 |
| `GITHUB_APP_PRIVATE_KEY` | GitHub App 認証用秘密鍵（PEM） | App 作成時にダウンロード |
| `GITHUB_OWNER` | デフォルト owner（未設定時は `hummer98` にフォールバック） | — |
...
**Fly Secrets への設定（GitHub App のみ・1回だけ）:**
...
#### 5.3.3 旧 Fly Secrets（廃止予定 — 参照されていない）

過去に投入された以下の Fly Secrets は agent-server のいずれの経路からも参照されず、`buildSanitizedEnv()` で opencode 起動時の env からも明示削除される。運用への影響を最小化するため削除はしないが、将来的に `flyctl secrets unset` する。

- `OPENROUTER_API_KEY`, `GEMINI_API_KEY`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `FIREBASE_TOKEN`

### 5.4 本番公開フロー

ゲストサイトの workspace 直下にある設定ファイルから自動振り分け:

```bash
# Cloudflare 経路 (workspace/wrangler.toml がある場合)
npx vite build
npx wrangler pages deploy dist --project-name=site-name
npx wrangler deploy functions/api/index.ts --name=site-name-api

# Firebase 経路 (workspace/firebase.json がある場合)
npx vite build
npm install --prefix functions    # firebase.json predeploy の tsc で必要
npx firebase deploy --non-interactive
```

Firebase トークンは `--token` フラグではなく env (`FIREBASE_TOKEN=...`) 経由で `firebase-tools` に渡し `ps` から漏れないようにしている。env は `secretsStore` から動的に組み立てる（`process.env` には設定しない）。
...
| ⑤ | 「本番公開」ボタンを押す | 設定ファイルから振り分け → Cloudflare（wrangler）または Firebase（firebase-tools）にデプロイ |
